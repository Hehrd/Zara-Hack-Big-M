...
import { Building2, LoaderCircle, MapPin, Sparkles, Trophy, TrendingUp, TrendingDown } from 'lucide-react'
...
export function RecommendationsPage() {
  ...
  useEffect(() => {
    ...
    loadGoogleMaps()
      .then(({ Map }) => {
        ...
        // deck.gl's GoogleMapsOverlay picks its render path from getRenderingType().
        ...
        // RASTER (deck's plain OverlayView path) unless a Map ID is configured.
  ...
  useEffect(() => {
    ...
    const layer = new GeoJsonLayer({
      ...
      stroked: true,
      filled: true,
      getFillColor: (f) => [...scoreColor(f.properties.t), 150],
    ...
    if (ranked[0]?.centroid && mapRef.current) {
  ...
  function handleSubmit(event) {
    ...
    if (!trimmed || !city.trim()) return
    recommend.mutate({ city: city.trim(), businessDescription: trimmed, requestedResultCount: 3 })
  }
  ...
  return (
    <div className="relative h-[calc(100vh-4rem)] min-h-[680px] overflow-hidden lg:h-screen">
      <div ref={mapElement} className="absolute inset-0 bg-[#dfe5dc]" aria-label="London LSOA opportunity heatmap" />
      ...
      <aside className="absolute inset-x-3 bottom-3 z-20 max-h-[78%] overflow-auto rounded-[24px] border bg-white/96 p-5 shadow-[0_24px_70px_-20px_rgba(14,35,27,.45)] backdrop-blur lg:inset-y-5 lg:left-5 lg:right-auto lg:max-h-none lg:w-[380px] lg:rounded-[28px] lg:p-6">
        <div className="mb-6">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Locus recommendations</p>
          <h1 className="mt-2 text-2xl font-semibold tracking-[-0.035em]">Where should you open?</h1>
          <p className="mt-2 text-sm leading-6 text-muted-foreground">Describe your business. We match it to London demographics and live places data, then rank the best LSOAs.</p>
        </div>
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          <label className="block space-y-2 text-sm font-medium">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="h-11 w-full rounded-md border border-input bg-white pl-10 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                placeholder="London"
              />
            </div>
          ...
          <label className="block space-y-2 text-sm font-medium">
            ...
            <div className="relative">
              ...
              <textarea
                ...
                rows={4}
                className="w-full rounded-md border border-input bg-white p-3 pl-10 text-sm leading-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                placeholder="e.g. A premium yoga studio targeting affluent professionals"
              />
            </div>
          </label>
          ...
          <Button type="submit" className="h-11 w-full" disabled={recommend.isPending || !description.trim()}>
            {recommend.isPending ? <><LoaderCircle className="animate-spin" /> Scoring London…</> : <><Sparkles /> Find best areas</>}
          </Button>
        </form>
        ...
        {!result && !recommend.isPending && (
          <div className="mt-6 rounded-2xl bg-[#f3f6f1] p-4">
            <div className="flex items-center gap-2 text-sm font-medium"><Sparkles className="size-4 text-emerald-700" /> How scoring works</div>
            <p className="mt-2 text-xs leading-5 text-muted-foreground">Each London LSOA is scored by weighting census layers (population, age, economic activity, deprivation) plus live competitor and relevant-place density from Google Maps. The map colors every area by its final score.</p>
          </div>
        )}
      </aside>

      {result && (
        <div className="absolute bottom-[calc(78%+24px)] right-4 z-20 w-48 rounded-2xl border bg-white/94 p-3 text-xs shadow-lg backdrop-blur lg:bottom-6 lg:right-6">

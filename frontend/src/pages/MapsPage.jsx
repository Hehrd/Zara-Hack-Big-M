...
import { Bookmark, Building2, Compass, Eraser, Flag, GitCompareArrows, LoaderCircle, MapPin, Rotate3D, Sparkles, Trophy, TrendingDown, TrendingUp, X } from 'lucide-react'
    ...
    <div className="relative h-[calc(100vh-4rem)] min-h-[680px] overflow-hidden lg:h-screen">
      ...
      {regionContextMenu && <>
        ...
        <div className="fixed z-50 w-56 overflow-hidden rounded-xl border bg-white p-1.5 text-sm shadow-2xl" style={{ left: regionContextMenu.x, top: regionContextMenu.y }} role="menu">
          ...
          <button type="button" role="menuitem" className="flex w-full items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs text-muted-foreground hover:bg-muted" disabled><Bookmark className="size-4" />Save region <span className="ml-auto text-[9px] uppercase tracking-wide">Soon</span></button>
        </div>
      </>}
